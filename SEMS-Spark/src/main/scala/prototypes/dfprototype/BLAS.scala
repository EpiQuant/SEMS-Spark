...
//import org.apache.spark.ml.linalg.{ DenseVector, DenseMatrix, Vector, Vectors, SparseVector, SparseMatrix}
...
object BLAS{
  ...
  private def axpy(a: Double, x: DenseVector, y: DenseVector): Unit = {
    val n = x.size
    _f2jBLAS.daxpy(n, a, x.values, 1, y.values, 1)
  }
  ...
  private def axpy(a: Double, X: DenseMatrix, Y: DenseMatrix): Unit = {
    ...
    _f2jBLAS.daxpy(X.numRows * X.numCols, a, X.values, 1, Y.values, 1)
  }
  ...
  private def dot(x: DenseVector, y: DenseVector): Double = {
    val n = x.size
    _f2jBLAS.ddot(n, x.values, 1, y.values, 1)
  }
  ...
  def scal(a: Double, x: Vector): Unit = {
    x match {
      case sx: SparseVector =>
        _f2jBLAS.dscal(sx.values.length, a, sx.values, 1)
      case dx: DenseVector =>
        _f2jBLAS.dscal(dx.values.length, a, dx.values, 1)
      case _ =>
 ...
 /* private def syr(alpha: Double, x: DenseVector, A: DenseMatrix) {
  ...
  def gemm(
    ...
    if (alpha == 0.0 && beta == 1.0) {
      ...
      _f2jBLAS.dscal(C.values.length, beta, C.values, 1)
    } else {
      A match {
